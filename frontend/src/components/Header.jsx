...
export default function Header({ dark, setDark }) {
  ...
  return (
    <header
      className={`flex items-center gap-3 px-6 py-3 justify-between 
                  transition-colors duration-700 ease-in-out
                  ${dark ? 'bg-[#21755c] text-black' : 'bg-[#21755c] text-white'}`}
    >
      <img src={Trilhar} alt="Trilhar" className="w-20 h-20 sm:w-24 sm:h-24 transition-all duration-700"/>
      ...
      <button
        onClick={toggleDark}
        className={`px-3 py-3 rounded-full font-semibold flex items-center justify-center 
